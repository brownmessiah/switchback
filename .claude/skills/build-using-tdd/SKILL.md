...
- **Verification commands:** `pnpm typecheck` (tsc --noEmit) and `pnpm test` (vitest run). For issues whose acceptance is browser/E2E, the implementer also runs the relevant Playwright project: `pnpm e2e --project <unauthenticated|customer|vendor|admin|cross-surface|i18n>`. E2E needs the dev server + `docker-compose.e2e.yml` (Meilisearch) up.
