...
**Default path:** 25% captured to Switchback via Razorpay at booking; the remaining 75% is auto-captured by Razorpay on a card-on-file at **T-24h**, aligned with the existing WhatsApp T-24h reminder so the customer sees the charge notification and the reminder in the same window. Switchback takes its commission on the **full booking value**, not just the advance, and pays the vendor net via the standard payout pipeline.
...
- **Cash/UPI direct to vendor on arrival** — keeps the legacy app's "no transactional integrity for the money path" problem, which the entire rebuild exists to fix. Switchback can't enforce a refund SLA on money it never held.
