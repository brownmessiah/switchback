...
import { listCategories, loadCategoryLanding } from '@/lib/activities/queries'
...
/**
 * `/category/{slug}` — a category rollup (all published Experiences whose
 * activity's `category` matches). Complements `/adventure/{slug}` and
 * `/activities/{slug}` with a self-canonical (via generateAlternates) so the
 * three families don't compete for the same indexing signal. Only non-empty
 * categories are emitted — `urban` (no activities) is excluded.
 */
export function generateStaticParams() {
  return listCategories().map((category) => ({ slug: category }))
}
...
export default async function CategoryLandingPage({
  ...
  // Categories use a localized display label (water → "Water sports" etc.).
  const categoryDisplay = t(`categories.${data.category}`)
  const pageTitle = t('heroTitle', { category: categoryDisplay })
  const experienceCount = data.experiences.length
  // First activity in the category seeds the hero image.
  const heroImageSlug = data.activities[0]?.slug ?? data.category

  const itemListJson = experienceCount
    ? itemList({
        name: t('itemList.name', { category: categoryDisplay }),
  ...
  const faqItems = [
    {
      question: t('faq.safetyQuestion', { category: categoryDisplay }),
      answer: t('faq.safetyAnswer'),
    },
    {
      question: t('faq.includesQuestion', { category: categoryDisplay }),
      answer: t('faq.includesAnswer'),
    },
    {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-12">
      ...
      <header className="mb-[var(--space-section)]">
        <div className="grid items-center gap-8 lg:grid-cols-[1.1fr_1fr]">
          <div>
            ...
            <p className="measure mt-4 flex items-start gap-2 text-lg text-foreground">
              <XCircle
                ...
                aria-hidden="true"
              />
              <span>{t('refundLead', { category: categoryDisplay.toLowerCase() })}</span>
            </p>
            <p className="measure mt-3 text-base text-muted-foreground">
              {t('editorialIntro', { category: categoryDisplay.toLowerCase() })}
            </p>
          ...
          <div className="relative aspect-[4/3] overflow-hidden rounded-[var(--radius-card)] shadow-[var(--shadow-md)] lg:aspect-[5/4]">
            <Image
              src={getActivityImage(heroImageSlug)}
              alt={pageTitle}
              fill
      ...
      {/* Category rollup experiences grid (across all activities in category). */}
      <section
        aria-label={t('experiences.sectionLabel', { category: categoryDisplay })}
        className="mb-[var(--space-section)]"
      >
        <div className="mb-4 flex flex-wrap items-baseline justify-between gap-2">
          <h2 className="font-heading text-h3 font-semibold tracking-tight">
            {t('experiences.heading', { category: categoryDisplay })}
          </h2>
          {experienceCount > 0 && (
        ...
        {experienceCount === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-[var(--radius-card)] border border-dashed py-16 text-center">
            <p className="text-lg font-medium">
              {t('experiences.emptyTitle', { category: categoryDisplay.toLowerCase() })}
            </p>
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const categoryDisplay = t(`categories.${data.category}`)
  return {
    title: t('metadata.title', { category: categoryDisplay }),
    description: t('metadata.description', { category: categoryDisplay }),
