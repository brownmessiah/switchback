...
/**
 * Vendor-onboarding profile-creation CORE (issue #03 review, FIX 5).
 *
 * db-injected, auth-free, integration-testable. Lives in a PLAIN (non-`'use
 * server'`) module so it is NOT exposed as a client-callable endpoint — every
 * exported async function in a `'use server'` file is a network endpoint, and a
 * core taking an arbitrary `userId` would be an IDOR. The thin Server Action
 * wrapper `createVendorProfileAction` in ./actions derives identity from the
 * session and delegates here (it stays UNGATED — it is the entry point for
 * profile-less users, correct per ADR-0006 design).
 */
