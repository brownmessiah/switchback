...
_Avoid_: Operator, host, supplier, partner, merchant. ("Operator" appears in the global research but Switchback uses "Vendor" consistently — matches the legacy schema and Indian-industry vocabulary.)
...
**Advance**:
The 25% of a Booking's total value that Switchback captures at booking time under the partial-payment model.
...
The 0.1% deduction Switchback must withhold on the gross Booking value paid to resident-Indian Vendors under Section 194-O of the Income Tax Act (rate reduced from 1% to 0.1% by Finance Act 2024, effective 1 Oct 2024). Filed quarterly via Form 26Q; Vendor receives Form 16A. Not optional. See ADR-0016.
...
**Commission**:
The percentage of a Booking's gross value that Switchback retains; deducted from the Payout. Realised at Booking Completion, not at payment capture.
_Avoid_: Fee, take rate, margin.

**Wallet**:
The customer-facing surface that displays a Customer's combined balances on Switchback. Backed by two separate buckets (Switchback credit + Refund balance). See ADR-0004.
_Avoid_: Account, balance, credits. ("Account" is the auth abstraction.)

**Switchback credit**:
Closed-loop promotional balance. Funded by referral credit, festive promo credit, loyalty rewards. Spendable only on Switchback Bookings; never cashable. Expires 12–18 months from issue.
...
**AI-assisted surface**:
Any Switchback UI element whose content is partially or fully generated by an LLM (review summaries, vendor listing drafts, inbox reply suggestions, trip planner recommendations). Always labelled to the user; always backed by an `ai_generations` audit row. See ADR-0010.
...
**Required permit**:
A state-issued legal document a Customer must obtain before participating in a permit-gated Experience (ILP, PAP, wildlife area entry). Switchback surfaces the requirement and links to the official process; Switchback does not obtain permits on behalf of Customers in v1.
...
**Launch locale**:
A locale where Switchback has populated content sufficient to ship to users at launch. At v1, only `en` and `hi` are launch locales; `ta, mr, bn` are infrastructure-supported but content rolls in across v1.x.
...
**Channel manager**:
A third-party booking-and-inventory platform (Bokun, FareHarbor, Rezdy) that a Vendor uses to manage their offerings across multiple sales surfaces. Switchback can connect to a Vendor's channel manager to mirror Experience content and reserve availability — never to ingest Bookings made elsewhere. See ADR-0014.
_Avoid_: Inventory system, partner platform.

**Channel-connected Vendor**:
A Vendor who has authorised Switchback to read from / write to their channel-manager account. Inherits `identity` KYC tier by default; promotion to Business-verified still requires the standard Switchback verification flow.
_Avoid_: Bokun Vendor, third-party Vendor.

**Diverged field**:
An Experience field that a Channel-connected Vendor has manually overridden on Switchback, taking it out of the sync flow. Subsequent channel-manager polls do not overwrite diverged fields.
...
_Avoid_: Safety features, emergency system. ("Emergency" is misleading — Switchback notifies, doesn't dispatch.)
...
_Avoid_: Emergency contact (semantically suggests Switchback will call them in an emergency, which is wrong — they're notified by us, they decide what to do).

**SOS event**:
A Customer-triggered notification fan-out during an active Booking. Records the trigger, attempts to capture location, sends WhatsApp + SMS to the trusted contact, the Vendor, and Switchback ops. Distinct from a Dispute — SOS is in-trip emergency notification; Dispute is post-trip service complaint.
...
- A **Required permit** is surfaced to the Customer at Booking time but obtained by the Customer themselves; Switchback does not act as broker in v1
...
> **Dev:** "A **Channel-connected Vendor** has their Bokun listing show a different price than what's on Switchback. Which wins?"
>
> **Shivam:** "Depends on whether the price field is **diverged**. By default Bokun is source-of-truth — nightly poll overwrites Switchback. But if the Vendor edited it on Switchback, the override sticks until they explicitly re-sync from the diverged-field UI."

> **Dev:** "A Customer on a multi-day trek triggers **SOS** from outside cell coverage; the geolocation call fails. What happens?"
>
> **Shivam:** "The **SOS event** still fans out — to the **Trusted contact**, the **Vendor**, and Switchback ops via WhatsApp + SMS. Location is included if we have it, omitted if not. The notification doesn't depend on geolocation success."
